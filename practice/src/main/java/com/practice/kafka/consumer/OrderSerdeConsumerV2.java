...
public class OrderSerdeConsumerV2<String extends Serializable, OrderModel extends Serializable> {
    ...
    private void addShutdownHookToRuntime(KafkaConsumer<String, OrderModel> kafkaConsumer) {
    ...
    private void pollConsumes(long durationMillis, java.lang.String commitMode) {
        try {
            while (true) {
    ...
    private void pollCommitAsync(long durationMillis) {
        ConsumerRecords<String, OrderModel> consumerRecords = kafkaConsumer.poll(Duration.ofMillis(durationMillis));
